...
const stunServers = [
    ...
    ,
        ...
        //   username: "b40957bac743a962f5ff45fa",
        //   credential: "seqyQXHSk31DzK05",
        // },
        // {
...
var micButton = document.querySelector("#btn-toggle-audio");
// console.log(btnJoin)
// set username
...
.then(stream => {
    ...
    btnToggleAudio.onclick = function(e){
        let buttonAudio = document.getElementById("btn-toggle-audio");
        let activeClass = buttonAudio.querySelector(".fa-microphone")
        let deactiveClass = buttonAudio.querySelector(".fa-microphone-slash") 
        
        audioTracks[0].enabled = !audioTracks[0].enabled;
        console.log(audioTracks[0].enabled);
        if(audioTracks[0].enabled === false){
            activeClass.style.display = "none"
            deactiveClass.style.display = "block"
            return;
        }
        if(audioTracks[0].enabled === true){
            activeClass.style.display = "block"
            deactiveClass.style.display = "none"
            return;
        }
        // else{
    ...
    btnToggleVideo.onclick = function(){
        let buttonVideo = document.getElementById("btn-toggle-video");

        let activeClass = buttonVideo.querySelector(".fa-video")
        let deactiveClass = buttonVideo.querySelector(".fa-video-slash") 
...
if(!localScreenSharing && !remoteScreenSharing){
    ...
    peer.oniceconnectionstatechange = () => {
        console.log("on ice state changed");
...
var robtoVideoContainer = document.querySelector('#robot-video-screen');
let isSameName = document.getElementById(`${peerUsername}-video`);
if(isSameName){
    isSameName.parentElement.remove();

}
// console.log("issame",isSameName);
...
robtoVideoContainer.appendChild(videoWrapper);


videoWrapper.appendChild(remoteVideo);
// hoverCameraContorller()

return remoteVideo;
}else{

    let otherVideoContainer = document.querySelector('.splide__list');
    const LI = document.createElement("li");
    LI.classList.add("splide__slide");
    LI.classList.add("remote-user")
    const pTag = document.createElement("p");
    pTag.textContent = peerUsername
    ...
    // videoWrapper.appendChild(pTag)
    LI.appendChild(pTag)
    LI.appendChild(remoteVideo)
    console.log("user", username);
    // add the wrapper to the video container
    otherVideoContainer.appendChild(LI);
    // LI.appendChild(otherVideoContainer)
...
const robotVideos = document.getElementById("robot-video");
...
if(robotVideos){
    ...
    else controller.style.display = "none"
}
}
const targetNode = document.querySelector('.splide__list');
...
const targetNodeRobot = document.querySelector('#robot-video-screen');
console.log(targetNodeRobot);
...
// Configuration of the observer:
const configRobot = { childList: true };

// Start observing the target node
observerRobot.observe(targetNodeRobot, configRobot);


sliderFunc()
// hoverCameraContorller()

// function hoverCameraContorller(){
//     const servo = document.querySelector(".servo")
//     const remoteUser = document.querySelector(".robot-screen .remote-user")
//     let parentElement = document.querySelector("#robot-video");
    
//     if(windowSize >= 786){
//         if(remoteUser){
//             parentElement.addEventListener("mouseover", (e)=> {
//                 servo.style.display = "flex";
//                 servo.style.opacity = 1;
    
    
//             }) 
//             servo.addEventListener("mouseover", (e)=> {
//                 servo.style.display = "flex";
//                 servo.style.opacity = 1;
//             })
...
function sliderFunc(){
    ...
    splide.mount();
    let prevNext = document.querySelectorAll(".splide__arrow");
    let liDiv = document.querySelectorAll(".others-user .remote-user");
    console.log(liDiv);
    if(liDiv.length > 1){
        prevNext.forEach(v=> v.style.display = "block")
        
    }else{
        prevNext.forEach(v=> v.style.display = "none")
...
// let isLandascape = false
// window.addEventListener("resize", (e)=> {
//     console.log(windowSize);
//     windowSize = window.innerWidth;
//     if(isLandascape === false && windowSize < 786){
//         console.log("not ok");
//     }
//     else{
//         console.log("ok");
...
//     isLandascape = e.matches;
//     console.log(isLandascape);
// })
// let controller_group = document.querySelector(".controller-group"); 
// let mainGrid = document.querySelector(".main-grid-container"); 

// window.addEventListener("resize", (e)=> {
//     console.log(mainGrid.clientWidth - 300);
//     controller_group.style.marginLeft = `${mainGrid.clientWidth - 300}px`
// })